...
bool ConnectionHandler::isPasswordCorrect()
{
	...
	if (bytesRead < 0)
	{
		error("ERROR reading password from socket");
	}
...
{
	if( strcmp(readPassword, password.c_str()) != 0)
	{
		int bytesWritten = write(outputSocket, (void*)MessageDictionary::passwordIncorrect.data(), bufferSize);
		if (bytesWritten <= 0)
			error("ERROR writing to socket");
		std::cout << "[ConnectionHandler] Unknown client rejected" << std::endl << std::endl;
		return false;
	}
	else
	{
		int bytesWritten = write(outputSocket, (void*)MessageDictionary::passwordCorrect.data(), bufferSize);
		if (bytesWritten <= 0)
			error("ERROR writing to socket");
...
{
	perror(message);
	//TODO some disconnection method
}
